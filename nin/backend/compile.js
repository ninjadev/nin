...
async function collect(projectPath, data, options) {
  ...
  const htmlPreamble =
    fs.readFileSync(projectPath + '/index.html', { encoding: 'utf8' })
      ...
      ga('create', '${projectSettings.googleAnalyticsID}', 'auto');
      ga('send', 'pageview');
      </script>` : '')
      .replace(
...
const compile = async function (projectPath, options) {
  ...
  if (!options.closureCompiler) {
    ...
    renderOK();
    await collect(projectPath, sources, options);
  } else {
    ...
    await collect(projectPath, out.compiledCode, options);
  }
